# initup documentation

Reference material for **using** initup and **authoring** plugins — for people and
...
2. **Add a plugin** — `initup add heroui` (or `initup add ./my-plugin.json`). See
   [using-plugins.md](./using-plugins.md).
3. **Author your own** — write `plugins/<name>/plugin.ts`, then
   `initup make plugins/<name>/plugin.ts`. See
...
| Doc | What it covers |
| --- | --- |
| [using-plugins.md](./using-plugins.md) | Full user guide: every CLI command and flag, adding first-party vs third-party plugins, trust tiers, the dry-run plan + consent gate, `--json`/`--silent` for scripts and AI, monorepo `--app`, and driving initup from the MCP server (the 6 tools + confirm-token handshake). |
...
| [execution-model.md](./execution-model.md) | How initup stays safe: reads-immediate/writes-deferred, the Plan, the consent gate + MCP confirm-token handshake, transaction/rollback, per-op idempotency, and the sandbox + capabilities model. |
